package com.artlite.ckconcept.ui.recycle.channels;
...
public class KitRecycleChannel extends BaseObject {
     ...
     */
    public KitRecycleChannel(@LayoutRes int layoutID,
        ...
        //Get required data
        this.channelName = KitChannelDetailsHelper.getChannelName(channel);
        this.channelShortName = KitChannelDetailsHelper.getChannelShortName(channel);
        this.isGroup = KitChannelDetailsHelper.isGroupChat(channel);
    }

    /**
     * Method which provide the getting the view for KitRecycleChannel
     *
    ...
    protected String getChannelDate() {
        return KitChannelDetailsHelper.getDate(channel.get());
    }
    ...
    protected String getLastMessage(@NonNull final Context context) {
        if (TextUtils.isEmpty(latestMessage)) {
            latestMessage = KitChannelDetailsHelper.getLastMessage(context,
                    channel.get(),
                    noMessagesText,
                    locationText,
                    poolMessage,
    ...
    protected void setChannelAvatar(@Nullable final Context context,
        ...
        KitChannelDetailsHelper.setChannelAvatar(context, imageView, getChannel());
    }
    ...
    protected Drawable getLastMessageTypeDrawable(@Nullable final Context context) {
        if (typeDrawable == null) {
            typeDrawable = KitChannelDetailsHelper.getLastMessageTypeDrawable(context, getChannel(),
    ...
    protected boolean isHaveUnread() {
        return KitChannelDetailsHelper.isHaveUnreadMessages(getChannel());
    }
    ...
    protected void clearUnread(@NonNull final ChannelDetail detail) {
        if ((detail != null) && (detail.getChannel() != null)) {
            KitChannelsCacheManager.getInstance().clearUnreadMessages(detail);
     ...
     * View class
     */
    private final class ObjectView extends BaseRecyclerItem<KitRecycleChannel> {
         ...
         */
        @Override
        public void setUp(@NonNull final KitRecycleChannel recycleChannel) {
            //Get values
         ...
         */
        @Override
        protected void onItemClick(@NonNull KitRecycleChannel object) {
