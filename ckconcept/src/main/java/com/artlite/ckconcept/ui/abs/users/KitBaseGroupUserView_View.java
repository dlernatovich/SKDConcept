...
import com.artlite.ckconcept.ui.recycle.users.KitRecycleSelectedUser;
...
abstract class KitBaseGroupUserView_View extends KitBaseGroupUserView_Callbacks {

    public KitBaseGroupUserView_View(Context context) {
        super(context);
    }

    public KitBaseGroupUserView_View(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public KitBaseGroupUserView_View(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    /**
     * KitGroupContactContract.ViewClass object
     */
    protected final KitGroupContactContract.ViewClass view = new KitGroupContactContract.ViewClass() {
        ...
        public void setSelectedUsers(@Nullable List<User> users) {
            ...
            if (users.size() > 0) {
                ...
                for (User user : users) {
                    recycleUsers.add(new KitRecycleSelectedUser(presenter.getSelectedUserLayout(),
                            user, avatarMethod));
                }
        ...
        public List<BaseObject> getGroupCells(@NonNull final List<MMXUserGroup> groups) {
            ...
            if (groups.size() > 0) {
                for (MMXUserGroup userGroup : groups) {
                    cells.add(new KitRecycleGroup(presenter.getGroupLayout(),
                            userGroup));
                }
            } else {
            ...
            for (User user : userList) {
                ...
                if ((lastName != null)
                    ...
                    if ((previousLetter == null) ||
                        ...
                        cells.add(new KitRecycleAlphabetDivider(presenter.getAlphabeticDividerLayout(),
                                letter));
                    }
                }
                ...
                if (isContainIgnored == false) {
                    if ((userID != null) && (selectedUserMap.containsKey(userID) == true)) {
                        cells.add(new KitRecycleUser(presenter.getUserLayout(),
     ...
     *
     * @see KitBaseGroupUserView#addIgnoringUsers(Collection)
     */
